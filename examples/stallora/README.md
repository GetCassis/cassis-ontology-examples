...
Stallora is our demo dataset, a pan-European marketplace on **Snowflake** with 15 tables, 13 joins and 10 governed metrics. This is what a finished ontology looks like, so copy the *patterns*, not the content. The tree lives in [`cassis/`](cassis/):

```text
cassis/
├── AGENTS.md                         # the modeling guide, written by the CLI
├── domains/
│   ├── README.md                     # root domain: global rules every question inherits
│   ├── marketplace/
...
**The nav block at the bottom of a domain is generated.** Everything between the `cassis:nav` markers lists the domain's tables and metrics, and `cassis ontology fmt` writes it. Leave it alone and write your prose above it. A stale nav block fails the check.
...
**Joins have descriptions, and the descriptions do real work.** `joins.yml` warns about the fan-out on `GEOLOCATION` ("NEVER join directly … collapse to one row per prefix first") and flags the partial bridge between acquisition and marketplace data. A join that silently multiplies rows is the classic wrong-number generator, so say it where the agent will read it.
...
(Official CLI: `pip install cassis-cli`, plus a `CASSIS_API_KEY`. Setup at [docs.getcassis.com/cli](https://docs.getcassis.com/cli/#auth).) Every file here is canonical, so the CLI check, `cassis ontology fmt --check`, and the `cassis / ontology validation` check on pull requests all pass.
...
- [`cassis/AGENTS.md`](cassis/AGENTS.md): how to write an ontology that makes the agent accurate
