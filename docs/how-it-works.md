...
**Your git repository is the source of truth.** The whole context lives as a YAML tree under `cassis/ontology/` in the connected repository:
...
- **Every successful import creates a new immutable published context version**, labeled `Synced from git`, recording the commit SHA it came from. Version history in Cassis is therefore a mirror of your default branch's history (the default branch must be named `main` — see the [prerequisites](getting-started.md#prerequisites)) for `cassis/ontology/` files.
...
A push to the repository's **default branch** that touches `cassis/` files triggers an import: Cassis reads the whole `cassis/ontology/` tree at the branch head, replaces its context with it, and publishes a new version. This is what production reads — the agent answering questions (in the app, over MCP, or in Slack) always uses the latest published version, so merging to the default branch is your deploy step.
...
The `cassis/branch/*` branches and `cassis/ontology-publish` are managed by Cassis — treat them as read-only ([more in the workflow guide](workflow.md#cassis-managed-branches)).

## The two check runs

Cassis posts two GitHub check runs so problems surface where you work:

- **`cassis / ontology validation`** — posted on the head commit of any pull request that touches `cassis/` files, whatever the branch. It catches format problems: every YAML file must parse, and the whole tree must survive a canonical round-trip (Cassis reads it and re-serializes it; the output must match byte-for-byte, which catches non-canonical formatting that would produce noisy diffs later). Green means "Ontology is valid" — but a few semantic rules (metric completeness, `domain_path` references) are only enforced at import, so a green check is not quite a guarantee that the merge will import; see the [troubleshooting table](workflow.md#troubleshooting). The local validator covers both.
...
|---|---|
| Main context | The default branch (`cassis/ontology/` at its head) |
